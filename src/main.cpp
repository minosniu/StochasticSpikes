...
void cameraLoop( void *dummy )
{
	...
	do
	{
		Sleep(1);	//We are waiting 10 ms = 0.01 s
		WaitForSingleObject( hMutexCamera, INFINITE );
		leftImg = cvQueryFrame(leftCapture);
		
		cvCvtColor(leftImg, leftImgHSV,CV_BGR2HSV);
		
		cvInRangeS(leftImgHSV, hsvMinGreen, hsvMaxGreen, binLeftImgGreen);   
		cvInRangeS(leftImgHSV, hsvMinCyan, hsvMaxCyan, binLeftImgCyan);
		
		
		cvResize(binLeftImgGreen,binLeftImgSmallGreen);
		cvResize(binLeftImgCyan,binLeftImgSmallCyan);

		//cvSmooth(binLeftImgSmallGreen, binLeftImgSmallGreen, CV_MEDIAN, 1, 0);
		//cvSmooth(binLeftImgSmallCyan, binLeftImgSmallCyan, CV_MEDIAN, 1, 0);
		

		cvFlip(binLeftImgSmallGreen, binLeftImgSmallGreen,1); 
		cvFlip(binLeftImgSmallCyan, binLeftImgSmallCyan,1); 
		
		for(int i=0; i<NCX; i++)	
		{
			for(int j=0; j<NCY; j++)
			{
				VfnCam[j][i]= (binLeftImgSmallGreen->imageData[i*binLeftImgSmallGreen->widthStep+j] == 0) ? 0.0 : 1.0 ;
				CfnCam[j][i]= (binLeftImgSmallCyan->imageData[i*binLeftImgSmallCyan->widthStep+j] == 0) ? 0.0 : 1.0 ;
			}
		}
		

		//cvFlip(binLeftImgSmallGreen, binLeftImgSmallGreen,1); 
		//cvShowImage("BinLeftCam1",binLeftImgSmallGreen);
		//
		//cvFlip(binLeftImgSmallCyan, binLeftImgSmallCyan,1); 
		//cvShowImage("BinRightCam1", binLeftImgSmallCyan);

		ReleaseMutex( hMutexCamera );

	}while((leftImg && !kill));//|(rightImg && !kill));
	printf("\n\nEnded Camera Loop\n\n");
}
#endif
...
//2	globalpositionX
//3	globalpositionY
//4 estimated_pos[0]*NCX
//5 estimated_pos[1]*NCY
//13 t	(counter for Vfn and Cfn)
...
//6 global_forceY
//7 global_forceZ
//8 global_joint_anglesX
//9 global_joint_anglesY
//10 global_joint_anglesZ
//11 estimated_pos[0]*NCX
//12 estimated_pos[1]*NCY
//13 t	(counter for Vfn and Cfn)

////ONLY ONCE
//14 vV[i_IPC]=exp(-pow(vpoints[i_IPC]-.5,2)/pow(.15,2));
//15 Vfnx = 20*cos(.007*6.28*t); Vfny = 20*sin(.004*6.28*t);
//16 vC[i_IPC]=exp(-pow(vpoints[i_IPC]-.5,2)/pow(.07,2));
//17 VCnx = 0; VCny = 0;
//Use the following tag for parameters from 13 onwards
//COPYTHIS
